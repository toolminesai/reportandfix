...
export default async function ReportPage({ searchParams }: { searchParams: Promise<{ location?: string }> }) {
  ...
  const { data: resolved } = token ? await supabase.rpc('resolve_qr_location', { p_qr_token: token }) : { data: null }
  const location = Array.isArray(resolved) ? resolved[0] : resolved
