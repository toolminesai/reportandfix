...
export async function GET(request: Request) {
  ...
  if (profileError || !profile) return NextResponse.redirect(new URL('/auth?error=account', url.origin))
  if (profile.is_suspended) return NextResponse.redirect(new URL('/auth?error=suspended', url.origin))
